...
import me.odedniv.osafe.extensions.PREF_GENERATE_LENGTH
import me.odedniv.osafe.extensions.PREF_GENERATE_RULE_NOT_CONSECUTIVE
import me.odedniv.osafe.extensions.PREF_GENERATE_RULE_THREE_SYMBOL_TYPES
import me.odedniv.osafe.extensions.PREF_GENERATE_TYPE
...
class GeneratePassphraseDialog : AppCompatDialogFragment() {
  ...
  override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
    ...
    checkPassphraseRuleThreeSymbolTypes =
      layout.findViewById(R.id.check_passphrase_rule_three_symbol_types)
    checkPassphraseRuleNotConsecutive =
      layout.findViewById(R.id.check_passphrase_rule_not_consecutive)
  ...
  private fun setDefaults() {
    checkPassphraseRuleThreeSymbolTypes!!.isChecked =
      requireContext().preferences.getBoolean(PREF_GENERATE_RULE_THREE_SYMBOL_TYPES, false)
    checkPassphraseRuleNotConsecutive!!.isChecked =
      requireContext().preferences.getBoolean(PREF_GENERATE_RULE_NOT_CONSECUTIVE, false)
    try {
    ...
    setPassphraseLengthDefaults()
    seekPassphraseLength!!.progress =
      requireContext()
        .preferences
        .getInt(PREF_GENERATE_LENGTH, seekPassphraseLength!!.progress + 1) - 1
    textPassphraseLength!!.text = (seekPassphraseLength!!.progress + 1).toString()
  }

  private fun regeneratePassphrase() {
    textGeneratedPassphrase!!.text =
      generatePassphrase(
        type = PASSPHRASE_TYPES[spinnerPassphraseType!!.selectedItemPosition],
        length = seekPassphraseLength!!.progress + 1,
        rules =
          EnumSet.noneOf(PassphraseRule::class.java).apply {
            if (checkPassphraseRuleThreeSymbolTypes!!.isChecked)
              add(PassphraseRule.THREE_SYMBOL_TYPES)
  ...
  private fun initializeEvents() {
    var first = true
    spinnerPassphraseType!!.onItemSelectedListener =
    ...
    seekPassphraseLength!!.setOnSeekBarChangeListener(
      object : SeekBar.OnSeekBarChangeListener {
        override fun onProgressChanged(seekBar: SeekBar?, progress: Int, fromUser: Boolean) {
          context!!.preferences.edit().putInt(PREF_GENERATE_LENGTH, progress + 1).apply()
