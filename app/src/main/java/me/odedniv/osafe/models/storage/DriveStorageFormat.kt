...
class DriveStorageFormat(
  ...
  override fun conflicts(): Task<Boolean> {
    ...
    return query.onSuccessTask { metadata ->
      // drive needs update, and remote file was updated since the last update from this
      // resourceClient
      Tasks.forResult(
        metadata != null &&
  ...
  override fun read(): Task<ByteArray?> {
    ...
    return query.onSuccessTask<ByteArray?> { metadata ->
      metadata ?: return@onSuccessTask Tasks.forResult(null)
      Tasks.call(
        executor,
        Callable {
          client.files().get(metadata.id).executeMediaAsInputStream().use { it.readBytes() }
        },
      )
    }
  }
  ...
  override fun write(content: ByteArray): Task<Unit> {
    context.preferences.edit().putBoolean(PREF_DRIVE_NEEDS_UPDATE, true).apply()
    ...
    return query
      ...
      .addOnSuccessListener { fileId ->
        val now = Date()
        _queried = true
        _driveFileMetadata = DriveFileMetadata(id = fileId, modifiedDate = now)
        context.preferences
          .edit()
  ...
  override fun clear(): Task<Unit> {
    ...
    return query
      .onSuccessTask { metadata ->
        Tasks.call(executor, Callable { client.files().delete(metadata!!.id).execute() })
      }
  ...
  private data class DriveFileMetadata(val id: String, val modifiedDate: Date)
